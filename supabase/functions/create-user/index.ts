...
//       return new Response(
//         JSON.stringify({
//           error: { message: 'Missing required fields: email, password, userData' },
//         }),
//         {
//           status: 400,
//           headers: { ...corsHeaders, 'Content-Type': 'application/json' },
...
serve(async (req) => {
  ...
  try {
    ...
    const body = await req.json()
    const { email, password, userData, orgId } = body

    // Validate required fields
    if (!email || !password || !userData || !userData.role) {
      return new Response(
        JSON.stringify({
          error: { message: 'Missing required fields: email, password, userData (with role)' },
        }),
        {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    ...
    if (orgId) {
      ...
      if (orgError || !orgExists) {
        return new Response(
          JSON.stringify({ error: { message: 'Organization not found' } }),
          {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          },
    ...
    switch (userData.role) {
      ...
      case 'doctor': {
        // Organization doctor - store in field_doctors table
        const { error } = await supabaseAdmin.from('field_doctors').insert({
          auth_user_id: userId,
          name: userData.name,
          specialization: userData.specialization,
    ...
    // 4. Add organization mapping if orgId is provided and role is org-specific
    if (orgId) {
      const mappingRole = userData.role === 'field_doctor' ? 'doctor' : 
                         userData.role === 'org_admin' ? 'org_admin' : 'patient'
...
console.log(data)
      if (mappingError) {
        console.error('Organization mapping error:', mappingError)
        return new Response(JSON.stringify({ error: mappingError }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })
      }
