...
const PatientDetailsWrapper: React.FC<PatientDetailsWrapperProps> = ({ view,patient_id }) => {
  ...
  // Fetch visits when switching to history view
  useEffect(() => {
    if (activeView === 'history' && patient_id) {
      loadVisits(patient_id)
    }
  ...
  // Shared helpers from patient/history.tsx
    ...
    <ScrollView contentContainerStyle={styles.profileScrollContent}>
      <View style={styles.profileBlock}>
        ...
        {(patient.allergies || patient.current_medications || patient.medical_history) && (
          <View style={styles.profileSection}>
            <Text style={styles.profileSectionTitle}>Medical Information</Text>
            {patient.allergies && (
              <View style={styles.profileRow}>
        ...
        {(patient.emergency_contact_name || patient.emergency_contact_phone) && (
          <View style={styles.profileSection}>
            ...
            {patient.emergency_contact_name && (
              <View style={styles.profileRow}>
                <Text style={styles.profileLabel}>Name</Text>
                <Text style={styles.profileValue}>{patient.emergency_contact_name}</Text>
              </View>
            )}
            {patient.emergency_contact_phone && (
              <View style={styles.profileRow}>
                <Text style={styles.profileLabel}>Phone</Text>
                <Text style={styles.profileValue}>{patient.emergency_contact_phone}</Text>
              </View>
            )}
          </View>
        )}
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView 
        ...
        showsVerticalScrollIndicator={false}
        refreshControl={activeView !== 'history' ? <RefreshControl refreshing={refreshing} onRefresh={onRefresh} /> : undefined}
      >
        {renderTabs()}
...
const styles = StyleSheet.create({
  ...
  refreshButton: {
    width: 40, height: 40, borderRadius: 20, backgroundColor: '#F0F7FF',
  ...
  centerText: { marginTop: 12, color: '#666666', fontSize: 16, textAlign: 'center' },
})
